    ...
    <div class="form">
      ...
      <form>
        <div class="input-fields">
          <input required type="text" id="message" name="message" placeholder="Enter message"/><br/>
        </div>
        <div v-if="displayWarningCheckbox">
          <label for="warning">Warning message</label><br/>
...
export default {
  ...
  methods: {
    ...
    async submit(e) { // TODO: put in store??
      ...
      if (userRole === 'admin' || userRole === 'moderator' && document.getElementById("warning").checked === true) {
        post.warning = 1;
      } else {
        post.warning = 0;
      }
